...
const makeTemplate = () => { // 템플릿 생성 함수
  ...
  if (type === 'html') {
    ...
    console.error('html 또는 express-router 또는 가위바위보 중 하나를 입력하세요.');
  }
};
...
const playGame = (answer) =>{
  ...
  if (answer === computerChoice) {
    ...
    (answer === '가위' && computerChoice === '보') ||
    (answer === '바위' && computerChoice === '가위') ||
    (answer === '보' && computerChoice === '바위')
  ) {
    result = '이겼습니다!';
  } else {
    result = '졌습니다!';
  }
  console.log(`당신: ${answer}`);
...
const typeAnswer = (answer) => { // 템플릿 종류 설정
  if (answer !== 'html' && answer !== 'express-router' && answer !== '가위바위보') {
    console.clear();
    console.log('html 또는 express-router 또는 가위바위보만 지원합니다.');
    return rl.question('어떤 템플릿이 필요하십니까? ', typeAnswer);
  }

  if(answer == '가위바위보') {
    return rl.question('가위, 바위, 보 중 하나를 선택하세요:', playGame);
  }
  else {
    type = answer;
    return rl.question('파일명을 설정하세요. ', nameAnswer);
...
const program = () => {
  if (!type || !name) {
    ...
    console.clear();
    rl.question('어떤 템플릿이 필요하십니까? *html *express-router *가위바위보:', typeAnswer);
  } else {
    makeTemplate();
  }
};
